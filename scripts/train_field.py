...
def train(cfg: DictConfig):
    ...
    for iteration in (progress := trange(cfg.num_iterations)):
        optimizer.zero_grad()
        samples = generate_random_samples(dataset.d_coordinate, cfg.batch_size, device) # N, D
        predicted = field(samples) # N, D_out
        ground_truth = dataset.query(samples) # N, D_out
        ...
        if iteration % cfg.visualization_interval == 0:
            with torch.no_grad():
                ...
                samples = sample_grid(dataset.grid_size, device) # *dimensions, D
                *dimensions, d_coordinate = samples.shape
                samples = samples.reshape(-1, d_coordinate) # N, D
                values = [field(batch) for batch in samples.split(cfg.batch_size)]
                values = torch.cat(values).reshape(*dimensions, -1) # *dimensions, D_out
